...
export default function LessonScreen({ lesson, galaxy, subworld, onBack, onComplete, onWordLearned }) {
  ...
  const TIERS = {
    ...
    explore:    { label: 'Explore',    color: '#3AB5D4', desc: 'Deeper understanding — patterns, connections, context' },
    ...
    <div style={{ position:'fixed',inset:0,background:'#06040E',overflowY:'auto' }}>
      ...
      <div style={{ position:'relative',zIndex:1,paddingBottom:90 }}>
        ...
        <div style={{ padding:'14px 18px 10px',position:'sticky',top:0,zIndex:20,background:'linear-gradient(to bottom, rgba(6,4,14,.98) 85%, transparent)' }}>
          ...
          <div style={{ fontSize:10,color:`${color}99`,fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',marginBottom:5 }}>
            {galaxy.name} › {subworld?.name} › Ages {lesson.ageRange}
          </div>
          ...
          <div style={{ display:'flex',gap:5 }}>
            {Object.entries(TIERS).map(([key,t])=>(
              <button key={key} onClick={()=>setTier(key)} style={{ flex:1,padding:'7px 4px',borderRadius:11,cursor:'pointer',fontFamily:'Inter, sans-serif',fontSize:11,fontWeight:700,letterSpacing:'.04em',transition:'all .2s',background:tier===key?`${t.color}22`:'rgba(255,255,255,.04)',border:`1.5px solid ${tier===key?t.color:'rgba(255,255,255,.08)'}`,color:tier===key?t.color:'rgba(255,255,255,.35)' }}>
                {t.label}
              </button>
            ))}
          </div>
            ...
            <div style={{ background:`${stage.color}10`,border:`1px solid ${stage.color}28`,borderRadius:15,padding:'13px 15px' }}>
              ...
              {stage.id==='apply'&&(
                <div style={{ background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:12,padding:'12px 14px' }}>
                  ...
                  <p style={{ fontSize:13,color:'rgba(255,255,255,.65)',lineHeight:1.65 }}>{lesson.whyItMatters||`Understanding ${lesson.title?.toLowerCase()} helps you make better decisions and see the world with deeper understanding.`}</p>
                </div>
              )}
            ...
            <div style={{ background:'rgba(58,181,212,.07)',border:'1px solid rgba(58,181,212,.18)',borderRadius:15,padding:'12px 15px',marginBottom:14 }}>
              <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8 }}>
                <div style={{ display:'flex',alignItems:'center',gap:7 }}>
                  <div style={{ width:27,height:27,borderRadius:'50%',background:'rgba(58,181,212,.18)',display:'flex',alignItems:'center',justifyContent:'center' }}>
                    <Ic n="voice" s={13} c="#3AB5D4"/>
                  </div>
          ...
          {lesson.mastery?.badge&&(
            <div style={{ background:'rgba(255,215,60,.05)',border:'1px solid rgba(255,215,60,.15)',borderRadius:14,padding:'12px 15px',display:'flex',alignItems:'center',gap:12 }}>
              ...
              <div style={{ marginLeft:'auto' }}>
                <Placeholder label={lesson.mastery.badgeImage||'[BADGE_IMAGE]'} height={44} width={44} style={{ borderRadius:'50%',fontSize:7 }}/>
              </div>
            </div>
          )}
        </div>
