...
export default function OnboardingScreen({ onComplete }) {
  ...
  function handleNext() {
    if (isLast) { onComplete({name:name.trim()||'Learner',age,avatar,language:lang,onboardingComplete:true}) }
    else { setStep(s=>s+1) }
  }
    ...
    <div style={{position:'fixed',inset:0,background:'radial-gradient(ellipse at 50% 0%, #0E0830 0%, #06040E 70%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:28,overflowY:'auto'}}>
      ...
      <div style={{width:'100%',maxWidth:380,textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:10}}>
          {step>0&&(
            <button onClick={()=>setStep(s=>s-1)} style={{flex:1,padding:'14px',borderRadius:999,background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.15)',color:'rgba(255,255,255,.6)',fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>Back</button>
          )}
          <button onClick={handleNext} disabled={!canNext} style={{flex:2,padding:'14px',borderRadius:999,background:canNext?'#7C6AE0':'rgba(255,255,255,.08)',border:'none',color:canNext?'white':'rgba(255,255,255,.25)',fontSize:14,fontWeight:700,cursor:canNext?'pointer':'default',fontFamily:'Inter,sans-serif',boxShadow:canNext?'0 4px 20px rgba(124,106,224,.5)':'none',transition:'all .2s'}}>
