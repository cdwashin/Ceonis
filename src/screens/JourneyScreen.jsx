      ...
      <div style={{position:'fixed',inset:0,bottom:72,background:C.bg,overflowY:'auto'}}>
        <div style={{padding:'20px 20px 40px',maxWidth:680,margin:'0 auto'}}>
          ...
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:17,padding:'13px 15px',marginBottom:12}}>
            ...
            <div style={{height:7,background:'rgba(255,255,255,.1)',borderRadius:999,overflow:'hidden',marginBottom:5}}>
  ...
  return (
    <div style={{position:'fixed',inset:0,bottom:72,background:C.bg,overflowY:'auto'}}>
      <div style={{padding:'28px 20px 40px',maxWidth:680,margin:'0 auto'}}>
          ...
          <div style={{display:'flex',alignItems:'flex-end',gap:5,height:64}}>
            {weekly.map((d,i)=>(
              <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                <div style={{width:'100%',borderRadius:'3px 3px 0 0',minHeight:3,background:d.xp>0?'linear-gradient(to top,#7C6AE0,#B0A0FF)':'linear-gradient(to top,rgba(124,106,224,.2),rgba(124,106,224,.05))',height:`${Math.round((d.xp/wMax)*50)+3}px`}}/>
        ...
        {worldStats.map((ws,i)=>(
          <div key={ws.id} onClick={()=>setDetail(ws)}
            style={{padding:'13px 15px',background:C.surface,border:`1px solid ${C.border}`,borderRadius:18,cursor:'pointer',transition:'all .2s',display:'flex',alignItems:'center',gap:12,marginBottom:8,animation:`fadeUp .35s ${i*.03}s ease both`}}
