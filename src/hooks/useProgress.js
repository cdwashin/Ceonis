...
const DEFAULT_PROFILE = {
  ...
  voiceRate: 1, voicePitch: 1, voiceVolume: 1, selectedVoiceURI: null,
...
function mergeProgress(stored) {
  return {
    ...
    profile: {
      ...DEFAULT_PROFILE, ...(stored.profile || {}),
      accessibility: { ...ACCESSIBILITY_DEFAULTS, ...(stored.profile?.accessibility || {}) },
    },
  }
}
...
function computeStreak(lastActiveDate, currentStreak, longestStreak) {
  ...
  let streak = lastActiveDate === todayStr ? currentStreak
...
export function useProgress() {
  ...
  const completeLesson = useCallback((worldId, lessonId, score, lessonXp) => {
    const xpEarned = lessonXp ? Math.max(10, Math.round((score/100)*lessonXp)) : Math.max(10, Math.round((score/100)*100))
    const lessonsCompleted = { ...data.lessonsCompleted, [worldId]: { ...(data.lessonsCompleted[worldId]||{}), [lessonId]: { score, completedAt: Date.now(), xpEarned } } }
    let xp = (data.profile.xp||0) + xpEarned, level = data.profile.level||1, xpNext = data.profile.xpNext||1000
    ...
    const todayStr = today()
    const dailyLog = { ...data.dailyLog, [todayStr]: { xp:(data.dailyLog[todayStr]?.xp||0)+xpEarned, lessons:(data.dailyLog[todayStr]?.lessons||0)+1, minutes:data.dailyLog[todayStr]?.minutes||0 } }
  ...
  const learnWord = useCallback((wordId) => {
    ...
    save({ ...data, vocabularyLearned: { ...data.vocabularyLearned, [wordId]: existing ? { ...existing, reviewCount:(existing.reviewCount||0)+1 } : { learnedAt:Date.now(), reviewCount:1 } } })
  }, [data, save])
  ...
  const totals = {
    xp: data.profile.xp||0, level: data.profile.level||1, xpNext: data.profile.xpNext||1000,
    streak: data.profile.streak||0, longestStreak: data.profile.longestStreak||0,
  ...
  return { profile:data.profile, lessonsCompleted:data.lessonsCompleted, vocabularyLearned:data.vocabularyLearned, badgesEarned:data.badgesEarned, dailyLog:data.dailyLog, totals, updateProfile, updateAccessibility, completeLesson, learnWord, earnBadge, getWorldPct, getWeeklyActivity }
}
